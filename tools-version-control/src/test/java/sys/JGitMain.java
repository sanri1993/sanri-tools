...
//    }
     ...
     */
    @Test
    public void test15() throws IOException, GitAPIException {
        String dir = "D:\\test\\sanri-tools-maven";
        final File repositoryDir = new File(dir);
        Git git = Git.open(repositoryDir);
        // 3638bde921674107f45557212e57b6b804f1df0d 第一次提交
        // 181abaa2d2d04eb414ea37e27deb20bd5ef5c0a5 倒数第二次提交
        // ee9f9ac843f21d6d851ac39365fb365a4a8e4b7d 最后一次提交
        String commitId = "ee9f9ac843f21d6d851ac39365fb365a4a8e4b7d";

        final Repository repository = git.getRepository();
        RevWalk walk = new RevWalk(repository);
        final ObjectId versionId = repository.resolve(commitId);
        RevCommit choseCommit = walk.parseCommit(versionId);
        walk.markStart(choseCommit);
        ...
        if (next == null){
            System.out.println("当前是第一次提交 ");
            return ;
        }

        try(final ObjectReader objectReader = repository.newObjectReader()){
            CanonicalTreeParser oldTreeIter = new CanonicalTreeParser();
            oldTreeIter.reset(objectReader,choseCommit.getTree().getId());
            CanonicalTreeParser newTreeIter = new CanonicalTreeParser();
            newTreeIter.reset(objectReader,next.getTree().getId());

            final List<DiffEntry> diffEntries = git.diff().setNewTree(newTreeIter)
